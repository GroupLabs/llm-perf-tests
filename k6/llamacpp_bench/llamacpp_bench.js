// This code is forked from https://github.com/ggerganov/llama.cpp
// For more information, see https://github.com/ggerganov/llama.cpp/blob/master/examples/server/bench/README.md
//
// Modified to improve benchmark, and compatibility with tested server
...
// Max slot context
const n_ctx_slot = __ENV.SERVER_BENCH_MAX_CONTEXT
    ...
    JSON.parse(open(dataset_path))
      ...
      .map((data) => {
        return {
          ...
          n_completion_tokens: tokenizer(data["conversations"][1]["value"])
            .length,
        };
      })
      // Filter out too short sequences
      .filter(
        (conv) => conv.n_prompt_tokens >= 4 && conv.n_completion_tokens >= 4,
      )
...
const llamacpp_prompt_processing_second = new Trend(
  "llamacpp_prompt_processing_second",
);

const llamacpp_prompt_tokens_total_counter = new Counter(
...
const llamacpp_completions_truncated_rate = new Rate(
  "llamacpp_completions_truncated_rate",
);
const llamacpp_completions_stop_rate = new Rate(
...
export default function () {
  ...
  const res = sse.open(`${server_url}/completion`, params, function (client) {
  ...
  if (promptEvalTime > 0) {
    llamacpp_prompt_processing_second.add(
      (prompt_tokens / (promptEvalEndTime - startTime)) * 1e3,
    );
  }
